...
<?php } else { ?>
	...
	<?php foreach($formBuilder['formStructure']['fields'] as $fieldName => $fieldOptions) {
			...
			if($fieldType == 'submit' && $formBuilder['formDetails']['renderSubmitTag'] === false) {
				continue;
			}
			...
			<tr id="formRow_<?php echo $fieldName; ?>">
				...
				switch($fieldType) {
					...
					case 'radio': 
						foreach($fieldOptions['options'] as $key => $val) {
							echo $key; ?>: <input class="formField_<?php echo $fieldName; ?>" type="radio" name="<?php echo $fieldName; ?>" <?php echo $val == $fieldOptions['value'] ? 'checked="checked"': '' ; ?> value="<?php echo $val; ?>" <?php echo $extraAttributes; ?> /> &nbsp;&nbsp
					<?php 
						}	
...
<?php } ?>
<?php 
if($formBuilder['formDetails']['renderFormTag'] == true) { ?>	
</form>
<?php } ?>
</div>
